...
#pragma warning restore CS0618 // Type or member is obsolete
        AbstractRazorCohostLifecycleService? razorCohostLifecycleService) : ILspService, IOnInitialized, IDisposable
    {
        ...
        public Task OnInitializedAsync(ClientCapabilities clientCapabilities, RequestContext context, CancellationToken cancellationToken)
        {
            ...
            {
                // There are two initialization methods for Razor, which looks odd here, but are really controlled by the UI context
                // This method fires for any Roslyn project, but not all Roslyn projects are Razor projects, so the first UI context
                // triggers where there is a Razor file present in the solution, and the next is when one of those files is opened
                // in the editor. ie these two lines look the same, but really they do different levels of initialization.
                _razorFilePresentActivation = uIContextActivationService.ExecuteWhenActivated(Constants.RazorFilePresentUIContext, PreinitializeRazor);
