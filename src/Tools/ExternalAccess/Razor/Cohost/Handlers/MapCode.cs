// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.
// See the LICENSE file in the project root for more information.

using System.Threading;
using System.Threading.Tasks;
using Microsoft.CodeAnalysis.LanguageServer.Handler;
using Microsoft.CommonLanguageServerProtocol.Framework;
using LSP = Roslyn.LanguageServer.Protocol;

namespace Microsoft.CodeAnalysis.ExternalAccess.Razor.Cohost.Handlers;

internal static class MapCode
{
    public static Task<LSP.WorkspaceEdit?> MapCodeAsync(RazorCohostRequestContext? context, Solution solution, LSP.VSInternalMapCodeMapping[] mappings, bool supportsDocumentChanges, CancellationToken cancellationToken)
    {
        // Razor can't construct a RazorCohostRequestContext so we need to handle the null case, for their tests
        var logger = context is { } razorContext ? razorContext.GetRequiredService<ILspLogger>() : new EmptyLogger();

        return MapCodeHandler.MapCodeAsync(
            solution,
            mappings,
            supportsDocumentChanges,
            logger,
            cancellationToken);
    }
}
